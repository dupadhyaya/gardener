...
# 
load("Beginning.RData") # Load Data from the RD Objects

#Simple Regression
fw # DF with 2 columns
# Boxplots
boxplot(fw$count,fw$speed, names=c('count','speed'), col='gray')
title(' Box Plot of Count and Speed', xlab='Variable', ylab='Value')
boxplot(fw$count,fw$speed, names=c('count','speed'), col='gray', horizontal = T)
...
# Simplest form of regression is like regression
cor.test(~count + speed, data = fw)
lm(count ~ speed, data = fw) # response on the right of ~ lm(x~y)
?lm
# In regression we look for mathematical relationship between variables
...
speed2 = fw$speed
(count2 = 8.254 + 0.7913 * speed2)

fitted(fw.lm)  # almost same
fw$count  # Observed values
fitted(fw.lm) - fw$count  # this is difference predicted & actual values - errors

# Residuals 
residuals(fw.lm)  # another way to see the difference
# Absolute sum of these differences
sum(abs(residuals(fw.lm))) # this should be reduced for a model
...
# Exercise  - Cars ---------------------
str(cars)  # speed of the cars and time it takes to stop
